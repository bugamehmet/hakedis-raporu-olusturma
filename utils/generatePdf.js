...
// ADMİN İÇİN - HAKEDİSPDF ' LERİNİ OLUŞTURUYORUZ 

// USER İÇİN -- USERHAKEDİS OLANLAR USER KULLANICILARININ PDF OLUŞTURMA FONKSİYONLARI PDFLERİ OLUŞTURUP KAYDEDİYOR SONRASINDA BİRLESMİSPDF FONKSİYONU BUNLARI BİRLEŞTİRİP GERİ YOLLUYOR 

// SHOW PDF LER GEÇMİŞ HAKEDİŞLERİ GÖRÜNTÜLEME İÇİN 

// USERBİRLESMİŞ GEÇMİŞ HAKEDİŞLERİ İNDİRMEK İÇİN 
...
async function userkapakPDF(res, useridInfo, sirket_id) {
	const query =
		'SELECT * FROM hakedis_raporu WHERE kullanici_id = ? AND s_id = ? ORDER BY h_id DESC LIMIT 1';
	const params = [useridInfo, sirket_id];
	connection.query(query, params, (error, results) => {
		...
		doc.pipe(fs.createWriteStream('/Users/mehmet/hakedissaski/pdf/userkapak.pdf'));
		console.log('Hakediş raporu başarıyla oluşturuldu');
		doc.end();
	});
}
async function userraporPDF(
	res,
	useridInfo,
	gecikme,
	fark,
	...
	connection.query(query, params, (error, results) => {
		...
		doc.pipe(fs.createWriteStream('/Users/mehmet/hakedissaski/pdf/userrapor.pdf'));
		console.log('Hakediş raporu-2 başarıyla oluşturuldu');
		doc.end();
	});
}
async function useryapilanislerPDF(res, useridInfo, hakedis_tutari, sirket_id) {
	const query = 'select * from hakedis_3 where kullanici_id=? AND s_id=? order by h_id_3 desc';
	const params = [useridInfo, sirket_id];
	connection.query(query, params, (error, results) => {
		if (error) {
		...
		let x_hakedis_tutari_2 = parseInt(hakedis_tutari);
		let db_toplam = results
			.map((item) => item.Gas)
			.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
		let toplam = parseInt(db_toplam) + x_hakedis_tutari_2;
	...
	function pdf3_generate() {
		...
		connection.query(sql, params, (error, results) => {
			...
			doc.pipe(fs.createWriteStream('/Users/mehmet/hakedissaski/pdf/useryapilan.pdf'));
			console.log('Hakediş raporu-3 başarıyla oluşturuldu');			
			doc.end();
		});
	}
...
async function userbirlesmisPDF(
	res,
	useridInfo,
	gecikme,
	fiyat_farki,
	...
	await userkapakPDF(res, useridInfo, sirket_id);
	await userraporPDF(res, useridInfo, gecikme, fiyat_farki, var_yok, hakedis_tutari, kesinti, sirket_id);
	await useryapilanislerPDF(res, useridInfo, hakedis_tutari, sirket_id);

	const pdfFiles = ['./pdf/userkapak.pdf', './pdf/userrapor.pdf', './pdf/useryapilan.pdf'];
	const mergedPdfPath = './pdf/userbirlesmis.pdf';

	merge(pdfFiles, mergedPdfPath, function (err) {
		if (err) {
			console.error(err);
...
async function showkapakPDF(res, no, s_id) {
...
async function showraporPDF(res, no, s_id) {
	...
	connection.query(query, params, (err, results) => {
		...
		doc.pipe(fs.createWriteStream('/Users/mehmet/hakedissaski/pdf/rapor.pdf'));
		//doc.pipe(res);
		console.log('Hakediş raporu-2 başarıyla oluşturuldu');
		/*res.setHeader('Content-Type', 'application/pdf');
		res.setHeader(
...
async function showyapilanislerPDF(res, no, s_id) {
...
async function birlesmisPDF(res, no, s_id) {
	await showkapakPDF(res, no, s_id);
	await showraporPDF(res, no, s_id);
	await showyapilanislerPDF(res, no, s_id);

	const pdfFiles = ['./pdf/kapak.pdf', './pdf/rapor.pdf', './pdf/yapilan.pdf'];
	const mergedPdfPath = './pdf/birlesmis.pdf';
...
async function downloadinfoPDF(res, no, s_id) {
	await showkapakPDF(res, no, s_id);
	await showraporPDF(res, no, s_id);
	await showyapilanislerPDF(res, no, s_id);
